...
function sketchRNNStart() {
  ...
  for (let i = 1; i < rdpPoints.length-1; i++) {
    let strokePath = [rdpPoints[i].x - rdpPoints[i - 1].x, rdpPoints[i].y - rdpPoints[i - 1].y,1,0,0]
    seedPath.push(strokePath);
    };
  
  strokePath = [rdpPoints[rdpPoints.length-1].x - rdpPoints[rdpPoints.length-2].x, rdpPoints[rdpPoints.length-1].y - rdpPoints[rdpPoints.length-2].y,0,1,0]
  seedPath.push(strokePath);
  //console.log(seedPath);
  let csvContent = "data:text/csv;charset=utf-8,";

  seedPath.forEach(function(rowArray) {
